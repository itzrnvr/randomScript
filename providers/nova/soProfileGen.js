...
const Profile = require('./profileUtills');
...
const fetchNewUser = async () => {
  ...
  const getNewUserBeta = async () => {
    // Start fetching nextId in the background if usage > 1
    if (Profile.usage > 1 && !Profile.fetchingNextId) {
        Profile.fetchingNextId = true;
        fetchNewUser().then(newId => {
            Profile.nextId = newId;
            Profile.fetchingNextId = false;
        }).catch(error => {
    ...
    if (Profile.id === null) {
        Profile.id = await fetchNewUser();
        Profile.usage = 0;
    } else if (Profile.usage > 2 && Profile.nextId) {
        Profile.id = Profile.nextId;
        Profile.usage = 0
        Profile.nextId = null; // Clear nextId after using it
    }

    Logger.user(Profile.id, Profile.usage);
